...
metadata_for_pca_SIDfiltered %>%
  # group_by(RecruitmentCenter) %>%
  ...
  summarise(
    ...
    `Ever Cigarettes` = summaryCountPercent(cig_life, "1", inverse = T),
  ...
  # pivot_wider(id_cols = ` `, names_from = RecruitmentCenter) %>%
  # mutate(` ` = if_else(Aurora != "", paste0(". . . ", ` `), ` `)) %>%
