...
csl: https://raw.githubusercontent.com/citation-style-language/styles/d8423d84cd58eeb4f1e2c07d2b4102791d95b8f6/apa.csl
---
...
GSEA requires the input of both a ranked list of genes and a list of biological pathways with their associated genes. For ease of comparison with various pathways, genes were mapped from ENSEMBL ID's to ENTREZ (NCBI) ID's. All GSEA analysis was conducted using the R package `fGSEA` ver. `r package.version("fGSEA")`.
...
Only pathways with $\ge 2$ ENTREZ genes were considered for GSEA. After the initial analysis, we determined redundant pathways using the function `collapsePathways` from `fGSEA` [@korotkevich2016]. This report refers to non-redundant pathways as 'independent' pathways throughout the report. It should be noted that this terminology refers only to the idea that some gene-sets are represented in multiple pathways. In this case, independence does **not** refer to biological relationships between pathways.

Some definitions specific to these results provided by @subramanian2005 are as follows:
...
4228 Ensembl genes mapped to 3690 ENTREZ genes with 12.8% failing to map. All 3690 ENTREZ genes were included in GSEA.
...
```{r}
#Read in results
kegg_ind <- read_csv(here("DataProcessed/gsea_res/kegg_results_2022_07_05.csv"))
react_ind <- read_csv(here("DataProcessed/gsea_res/react_results_2022_07_05.csv"))
go_ind <- read_csv(here("DataProcessed/gsea_res/go_results_2022_07_05.csv"))
...
The remaining results are specific to each pathway collection. Only the top 10 pathways (determined by FDR) are presented. Full results can be found in the supplementary documentation.
...
A pattern in the results is that genes within a particular pathway are all regulated in the same direction (either all up- or all down-regulated). This relationship is expected. Leading edge genes are the genes from the gene set which appear in the ranked list at or before the point where the enrichment score reaches its maximum deviation from 0 (i.e. the enrichment score reported in the table above) [@subramanian2005]. Those genes will tend to cluster at one side. Pathways with reference to both cilium and the cornified envelope are recurrent in both the Reactome and GO collections. These along with some of the other pathways with FDR \< 0.05 are worthy of further investigation.
