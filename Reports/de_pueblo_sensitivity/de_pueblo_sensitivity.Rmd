...
The top 100 genes determined by the FDR-adjusted p-value were subset from Model A to compare to Model P. This comparison shows that the top 100 genes in Model A are identical to the top 100 genes in Model P. 
```{r fig.width=12}
#############Read in results###################
...
top_100_genes_AB[1:10,] %>% 
  ...
  mutate(FDR_A = if_else(FDR_A < 0.001, "< 0.001", as.character(round(FDR_A,3))),
         ...
         Gene = gsub("\\..*", "", Gene)) %>% 
  dplyr::rename(Percent_Difference = Estimate_P_Difference,
                FDR_P = FDR_B) %>% 
  kablize(.,digits = 3)
```
...
change_scatter <- top_1000 %>% 
  ggplot(aes(x = Estimate_A, y = Estimate_B)) +
  geom_point() +
  geom_abline(aes(intercept = 0, slope = 1, col = "Equal Estimates"), linetype = 2)+
  labs(x = "Estimate (Model A)",
       y = "Estimate (Model P)") +
  scale_x_continuous(breaks = seq(-10,10,2)) +
  scale_y_continuous(breaks = seq(-10,10,2)) +
  scale_color_manual(name = "Guides", values = c("Equal Estimates" = "red"))
change_scatter
```
