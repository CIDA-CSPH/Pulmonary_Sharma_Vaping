...
### Figure 3: $\beta$ and M-Value Distributions
...
MDS plots were made using the package `minfi` ver. `r package.version("minfi")` for both raw data and data pre-processed using dye-bias correction and background subtraction provided by the function `preprocessNoob`.

### Figure 5: Sample Clustering (Raw Data)
![](MDS_Raw_Plots_2.jpeg)

In raw plots, sample clustering can be seen by sex, but no clustering is seen by vape status or recruitment center. 

### Figure 6: Sample Clustering (Noob Data)
![](MDS_NOOB_Plots.jpeg)

Data processed using bye-bias correction and background subtraction show similar results to the raw data. Samples cluster only by sex. 
