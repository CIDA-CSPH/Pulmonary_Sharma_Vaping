...
```{r}
no_center <- read_csv(here("DataProcessed/methylation/results/noCenter_res_2022_12_01.csv"))

bacon_res <- read_csv(here("DataProcessed/methylation/results/full_res_bacon_2022_12_01.csv"))

noCenter_sig <- no_center[no_center$fdr < 0.05,]
bacon_sig <- bacon_res[bacon_res$fdr.bacon < 0.05, ]

no_center %>% 
  arrange(fdr)

par(mfrow = c(1, 2))
hist(no_center$p.value, main = "No Center",
     xlab = "p-value",
     ylim = c(0,40000))
hist(bacon_res$pval.bacon, main = "Bacon-Corrected", xlab = "p-value")
```
