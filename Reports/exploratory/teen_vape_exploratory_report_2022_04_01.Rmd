...
RUVr will be performed with k = 1 through k = 5 factors and the best k for factor analysis will be determined visually using an elbow plot, RLE plots, and dendrograms. Previous analyses used R package DESeq2 to fit the first-pass GLM. This analysis will use edgeR due to its reference in the literature for the RUVr procedure mentioned above.
...
par(mfrow = c(2,1))
plotRLE(ruv_ready, outline=FALSE, ylim=c(-4, 4), col = colors[metadata_joined_no12$vape_6mo_lab] )
legend(x = "bottomright", legend = c("Vaped", "Did Not Vape"), fill = c(colors[2], colors[1]))

plotPCA(ruv_ready, col = colors[metadata_joined_no12$vape_6mo_lab])
legend(x = "bottomright", legend = c("Vaped", "Did Not Vape"), fill = c(colors[2], colors[1]))
```
...
*Figure 7: RLE Comparison of RUVr Factor Inclusion*
...
```{r RUVr Continues, message=FALSE, warning=FALSE, fig.height=10, fig.width= 10}
#Make nicer plot
par(mfrow=c(5,1), cex = 0.5, cex.lab = 1.5, mar=c(2,5,0.5,0))

plotRLE(ruv_k1, outline=FALSE, ylim=c(-1,1), col = colors[metadata_joined_no12$vape_6mo_lab], ylab = "k = 1")
abline(h = c(.5, -.5), col = c("red", "red"))
legend(x = "bottomright", legend = c("Vaped", "Did Not Vape"), fill = c(colors[2], colors[1]))

plotRLE(ruv_k2, outline=FALSE, ylim=c(-1,1), col = colors[metadata_joined_no12$vape_6mo_lab], ylab = "k = 2")
abline(h = c(.5, -.5), col = c("red", "red"))
legend(x = "bottomright", legend = c("Vaped", "Did Not Vape"), fill = c(colors[2], colors[1]))

plotRLE(ruv_k3, outline=FALSE, ylim=c(-1,1), col = colors[metadata_joined_no12$vape_6mo_lab], ylab = "k = 3")
abline(h = c(.5, -.5), col = c("red", "red"))
legend(x = "bottomright", legend = c("Vaped", "Did Not Vape"), fill = c(colors[2], colors[1]))

plotRLE(ruv_k4, outline=FALSE, ylim=c(-1,1), col = colors[metadata_joined_no12$vape_6mo_lab], ylab = "k = 4")
abline(h = c(.5, -.5), col = c("red", "red"))
legend(x = "bottomright", legend = c("Vaped", "Did Not Vape"), fill = c(colors[2], colors[1]))

plotRLE(ruv_k5, outline=FALSE, ylim=c(-1,1), col = colors[metadata_joined_no12$vape_6mo_lab], ylab = "k = 5")
abline(h = c(.5, -.5), col = c("red", "red"))
legend(x = "bottomright", legend = c("Vaped", "Did Not Vape"), fill = c(colors[2], colors[1]))
...
sex_colors <- brewer.pal(3, "Set1")
center_colors <- brewer.pal(4, "Set3")

#assign color for each variable
#vape status
vape_status_dend_col <- vape_status_colors[vape_status]
sex_dend_col <- sex_colors[male]
center_dend_colors <- center_colors[as.factor(center)]


par(mar = c(12,4,1,1))
plot(calDend(ruv_k3), ylab = "k=3", ylim = c(0,0.8))
colored_bars(cbind(sex_dend_col, center_dend_colors, vape_status_dend_col), calDend(ruv_k3), rowLabels = c("Sex", "Recruitment Center", "Vape Status"))
legend(x = "topright", legend = c("Vaped", "Did not Vape"), fill = c(colors[2], colors[1]), title = "Vape Status")
legend(x = "top", legend = c("Male", "Female"), fill = c(sex_colors[2], sex_colors[1]), title = "Sex")
legend(x = "topleft", legend = c("Pueblo", "Denver", "Aurora"), fill = c(center_colors[3],center_colors[2], center_colors[1]), title = "Center")


par(mar = c(12,4,1,1))
plot(calDend(ruv_k4), ylab = "k=4", ylim = c(0,0.8))
colored_bars(cbind(sex_dend_col, center_dend_colors, vape_status_dend_col), calDend(ruv_k4), rowLabels = c("Sex", "Recruitment Center", "Vape Status")) 
legend(x = "topright", legend = c("Vaped", "Did not Vape"), fill = c(colors[2], colors[1]), title = "Vape Status")
legend(x = "top", legend = c("Male", "Female"), fill = c(sex_colors[2], sex_colors[1]), title = "Sex")
legend(x = "topleft", legend = c("Pueblo", "Denver", "Aurora"), fill = c(center_colors[3],center_colors[2], center_colors[1]), title = "Center")  
```

Based on the elbow plot, dendrograms, and RLE plots above, it appears that there is not significant improvement between K = 3 and K = 4. Analyses will proceed with K = 3 for RUVr. 
