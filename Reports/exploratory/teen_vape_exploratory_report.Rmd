...
Figure 3 identifies correlation patterns in the clinical variables of interest. To test for correlation, a contingency table was made for each comparison of variables. If the table contained any cells with <5 subjects, a Fisher's Exact test was used; otherwise, a Pearson's Chisq test was run. 
```{r}
source(here("Code/exploratory_report/01b_center_vape_correlation.R"), local = knitr::knit_global())

correlation_matrix_plot
```

From the correlation matrix, it appears that there may be significant correlation between vape status and recruitment center.   
...
### Figure 11: Dendrogram Comparison of RUVr Factor Inclusion (Sample 12 in dataset)
...
### Figure 12: Dendrogram Comparison of RUVr Factor Inclusion (Sample 12 removed)
```{r fig.height= 6, fig.width=10}
#assign color for each variable
#vape status
...
- Between this report and it's previous version ("teen_vape_exploratory_report_2022_04_06"), a mistake was corrected in gene-filtering. The previous report incorrectly filtered out genes with range > 100. This report correctly filters out gens with range < 100, which makes for significant differences in visualization and reporting. 
