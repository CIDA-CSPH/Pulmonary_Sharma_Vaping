...
### *Vape Status*  
Subjects are dichotomized to those that used a vaping device in the last 6 months and those who have not based on the variables *ever_vape*, *vape_days*, and *last_vape*. This variable will be referred to as *Vape Status* throughout this report. *Vape Status* could not be confirmed for one individual. Any analyses including *Vape Status* will have a sample size of **n = 50** assuming no missing values for the other included variables.  
&nbsp;  
Previous analyses showed **n = 12** participants had vaped in the last 6 months. This analysis will use **n = 13** participants who had vaped in the last 6 months. One participant (SID = 111) reported that they had used a vaping device 5 out of the last 30 days, but did not respond to *last_vape*. They were labeled as "NA" in previous analyses.  
...
Measures of lung function and IOS were visually inspected for normality using histograms. The lung function variable *`r colnames(tab1_dat[,12])`* and *`r colnames(tab1_dat[,13])`* reported 22 missing values. IOS measures *`r colnames(tab1_dat[,14])`* and *`r colnames(tab1_dat[,15])`* reported 1 and 6 missing values, respectively.    
...
# Results
After removing participants with missing values for *vape status*, we are left with **n = 50** subjects.  
...
The following figure contains demographic information presented in the table above. The majority of vaping subjects were recruited in Pueblo (92%) and identified as LatinX (85%). 

```{r Demographic Figure, include=T, fig.height=6, fig.width=9, fig.cap= "Demographic variables 'Gender' and 'LatinX' grouped by vaping status."}
#Demographic Figures
