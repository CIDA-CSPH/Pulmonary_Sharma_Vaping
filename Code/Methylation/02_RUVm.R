...
# Perform RUV stage 2 with RUV 4
rfit5 <- RUVfit(Y = M, X = grp, Z = z, ctl = ctl2,
                method = "ruv4", k = 1) # Stage 2 analysis
rfit6 <- RUVfit(Y = M, X = grp, Z = z, ctl = ctl2,
                method = "ruv4", k = 2) # Stage 2 analysis

rfit7 <- RUVadj(Y = M, fit = rfit5)
rfit8 <- RUVadj(Y = M, fit = rfit6)
