...
colnames(paths_res) <- c("Paths Tested",  "Genes Matched", "Non-redundant Pathways", "FDR < 0.05", "FDR < 0.1", "FDR < 0.2")
...
format_tab <- function(res_ind) {
  ...
  res_ind %>% 
    ...
    dplyr::mutate(ES = round(ES, 2),
                  ...
                  independent_path = if_else(is.na(independent_path), "Non-redundant", independent_path)) %>%
    dplyr::rename(Pathway = pathway,
                  "Non-redundant Pathway" = independent_path,
                  FDR = padj,
...
colnames(kegg_res_full) <- c("Pathway", "Non-redundant Pathway", "leadingEdge", "gene_name", "log2FoldChange", "regulation", "gene_level_pval", "gene_level_fdr", "path_level_fdr")
colnames(react_res_full) <- c("Pathway", "Non-redundant Pathway", "leadingEdge", "gene_name", "log2FoldChange", "regulation", "gene_level_pval", "gene_level_fdr", "path_level_fdr")
colnames(go_res_full) <- c("Pathway", "Non-redundant Pathway", "leadingEdge", "gene_name", "log2FoldChange", "regulation", "gene_level_pval", "gene_level_fdr", "path_level_fdr")

#Write out kegg results
list_of_datasets <- list("pathway_level" = kegg_res_supplement, "gene_level" = kegg_res_full)
writexl::write_xlsx(list_of_datasets, here("DataProcessed/gsea/kegg_full_res_2023_02_24.xlsx"))

#Write out react results
list_of_datasets <- list("pathway_level" = react_res_supplement, "gene_level" = react_res_full)
writexl::write_xlsx(list_of_datasets, here("DataProcessed/gsea/react_full_res_2023_02_24.xlsx"))

#Write out go results
list_of_datasets <- list("pathway_level" = go_res_suplement, "gene_level" = go_res_full)
writexl::write_xlsx(list_of_datasets, here("DataProcessed/gsea/go_full_res_2023_02_24.xlsx"))
